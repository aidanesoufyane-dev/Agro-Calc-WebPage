    ...
    <section className="ac-scene" aria-label="AgroCalc Pro product walkthrough">
      ...
      <div className="ac-scene-inner">
        ...
        <div className="ac-story-cta">
          <img className="ac-cta-static-phone" src="/screens/home.jpeg" alt="AgroCalc Pro home screen" loading="lazy" width="540" height="1170"/>
          <div className="ac-story-cta-heading"><div className="ac-eyebrow"><T en="FROM THE DEMO TO YOUR FIELD" fr="DE LA DÉMO À VOTRE TERRAIN"/></div><h2><T en={<><span className="ac-cta-line"><span>Your next </span><span>step.</span></span><em className="ac-cta-line"><span>In your </span><span>pocket.</span></em></>} fr={<><span className="ac-cta-line"><span>La prochaine </span><span>étape.</span></span><em className="ac-cta-line"><span>Dans votre </span><span>poche.</span></em></>}/></h2></div>
...
export default function LandingPage() {
  return <main className="ac-landing" id="landing-page"><Story/>
    ...
    <section className="ac-gallery ac-wrap" id="screenshots"><div className="ac-section-heading"><div><div className="ac-eyebrow"><T en="A CLOSER LOOK" fr="DE PLUS PRÈS"/></div><h2><T en={<>Every detail.<br/><em>One place.</em></>} fr={<>Chaque détail.<br/><em>Au même endroit.</em></>}/></h2></div><p><T en="Explore the actual AgroCalc Pro screens, from your first input to the finished report." fr="Découvrez les vrais écrans d’AgroCalc Pro, de la première saisie au rapport final."/></p></div><div className="ac-gallery-grid">{[['calculator', '01 / Formulation', 'fertilizer input form'], ['ionic', '02 / Analyse', 'ionic balance results'], ['bacs', '03 / A · B · C', 'A/B/C fertilizer tank distribution'], ['pdf', '04 / PDF', 'fertigation PDF report']].map(([screen, label, alt]) => <figure key={screen}><div className="ac-gallery-image"><img src={`/screens/${screen}.jpeg`} alt={`AgroCalc Pro ${alt}`} loading="lazy" width="540" height="1170"/></div><figcaption>{label}</figcaption></figure>)}</div></section>
